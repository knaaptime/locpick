...
def SCL(
    ...
    """Spatially Correlated Logit (SCL) factory.

    Dispatches to the appropriate concrete model based on which features
    are active:

    - ``SCL(...)``                                  -> ``MNL(..., graph=g)``
    - ``SCL(..., nests=N)``                         -> ``NestedMNL(..., graph=g)``
    - ``SCL(..., random_params=R)``                 -> ``MixedMNL(..., graph=g)``
    - ``SCL(..., nests=N, random_params=R)``        -> ``MixedNestedMNL(..., graph=g)``

    .. deprecated::
        Construct ``MNL`` / ``NestedMNL`` / ``MixedMNL`` / ``MixedNestedMNL``
        directly with ``graph=`` instead.
    """
    if graph is None:
        raise ValueError(
            "SCL requires a 'graph' argument specifying the spatial adjacency structure."
        )
    has_nests = nests is not None
    has_random = bool(random_params)
    ...
    if has_nests and has_random:
        ...
        return MixedNestedMNL(nests=nests, **mixed_extras, **common)
    if has_nests:
        from locpick.models.nested import NestedMNL

        return NestedMNL(nests=nests, **common)
    if has_random:
        from locpick.models.mixed import MixedMNL

        return MixedMNL(**mixed_extras, **common)
    from locpick.models.mnl import MNL
